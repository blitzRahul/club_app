...
export async function POST(req, res) {
    try {
        ...
        if(data.prompt.trim().length > WORD_LIMIT)
            ...
            prompt = data.prompt;
        candidate.responses.skills = prompt;

        // get response from sorting hat assistant
        const assistant = await openai.beta.assistants.retrieve('asst_gkjNQigUfa377a9mJdqSJ3MP')
...
export async function GET(req, res) {
    try {
        ...
        if(!candidate?.threads?.skills)
            return NextResponse.json({ error: "Not found" }, { status: 404 });

        if(candidate.threads?.skills?.message)
            return NextResponse.json({ message: candidate.threads.skills.message }, { status: 200 });

        const threadID = candidate.threads.skills.id;
        const thread = await openai.beta.threads.messages.list(threadID);

        const text = thread.data[0].content[0].text.value
        await Candidate.updateOne({ regNum: session.user.regNum }, { $set: { "threads.skills.message": text }})
