...
const TicketPage = () => {
    ...
    const getTicket = async () => {
        const response = await fetch(`/api/protected/ticket`, {
            method: 'POST',
    ...
    if (fetchStatus === 'loading') {
        ...
        return (
            <main className='bg-gray-950 flex flex-col h-max w-screen justify-start gap-10 items-center text-white p-8'>
                ...
                <p onClick={() => navigator.clipboard.writeText(ticket?.uuid) } className='text-[10px] cursor-pointer text-gray-400'>{ticket.uuid}</p>
            </main>
        )
    }
