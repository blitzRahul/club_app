...
function ChatPage() {
  ...
  const [submitted, setSubmitted] = useState(saveEnum.DEFAULT);
  ...
  const getCandidate = async () => {
    ...
    if (response.status == 200) {
      ...
      if (candidate.responses?.experience) {
        setInputValue(candidate.responses.experience);
        setDisabled(true);
        setSubmitted(saveEnum.SAVED);
      }
    } else {
      setDisabled(true);
    }
  ...
  const handleSendMessage = async () => {
    ...
    setDisabled(true);
    setSubmitted(saveEnum.SAVING);
    try {
      submitPrompt(promt);
    } catch (e) {
      console.log(e);
      setDisabled(false);
      setSubmitted(saveEnum.ERROR);
    }
  };
      ...
      <div className="flex flex-col md:flex-row w-full h-max justify-center items-center gap-10">
        <div className="flex flex-col w-full h-max justify-center items-center gap-10 text-center">
          ...
          <p className="text-base w-10/12">
            Hi 👋, write about specific things you have worked on in the past.
            This is a mandatory question and you cannot edit your response.
          </p>
          <textarea
            disabled={disabled}
            ...
            placeholder="organizing an event, managing finances, developing a site, writing content for a magazine or anything else you are proud of"
          />
