...
function ChatPage() {
  ...
  const getCandidate = async () => {
    ...
    if (response.status == 200) {
      ...
      if (candidate.responses?.communication) {
        setInputValue(candidate.responses.communication);
  ...
  const submitPrompt = async (prompt) => {
    const response = await fetch("/api/protected/gpt/communication", {
      method: "POST",
      ...
      <div className="flex flex-col w-full h-max justify-center items-center gap-10">
        <div className="flex flex-col w-full h-max justify-center items-center gap-10 text-center">
          ...
          <p className="text-base w-10/12">
            Hi 👋, write about why you would like to join and what innovative
            ideas you bring. This is a mandatory question and you cannot edit
            your response.
          </p>
          <textarea
            disabled={disabled}
            ...
            placeholder="I chose The Fusion Club because..., I have some ideas for a new event..., I expect the club to..."
          />
